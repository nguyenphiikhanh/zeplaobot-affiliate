...
const getStatusBadge = (status: string | null) => {
  if (status === "Completed")
    return {
      label: "Hoàn thành",
      class: "bg-emerald-50 text-emerald-600 border border-emerald-200/60",
    };
  if (status === "Cancelled")
    return {
      label: "Đã hủy",
      class: "bg-rose-50 text-rose-600 border border-rose-200/60",
    };
  if (status === "Unpaid")
    return {
      label: "Chờ thanh toán",
      class: "bg-blue-50 text-blue-600 border border-blue-200/60",
    };
  return {
    label: "Chờ duyệt",
    class: "bg-amber-50 text-amber-600 border border-amber-200/60",
  };
};
</script>
...
<template>
  <div class="w-full space-y-3 sm:space-y-4 text-left">
    <!-- Header Banner -->
    <div
      class="bg-white rounded-2xl p-4 sm:p-5 border border-slate-200/80 shadow-xs flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3"
    >
      <div class="space-y-0.5">
        <div
          class="flex items-center gap-2.5 text-slate-900 font-extrabold text-base sm:text-lg"
        >
          <div
            class="w-8 h-8 rounded-xl bg-orange-50 text-[#ee4d2d] flex items-center justify-center shrink-0"
          >
            <GiftOutlined class="text-base" />
          </div>
          <h2
            class="text-base sm:text-lg font-black text-slate-900 leading-none m-0"
          >
            Tra Cứu Đơn Hàng
          </h2>
        </div>
        <p class="text-xs text-slate-500 leading-relaxed">
          Theo dõi tiến trình ghi nhận hoa hồng và trạng thái duyệt đơn Shopee
          của bạn.
        </p>
      </div>
    ...
    <div
      class="bg-white rounded-2xl p-3 sm:p-5 border border-slate-200/80 shadow-xs space-y-3"
    >
      <!-- Search Input -->
    ...
    <!-- Orders List Card -->
    <div
      class="bg-white rounded-2xl p-3 sm:p-5 border border-slate-200/80 shadow-xs"
    >
      <div
        v-if="loading"
      ...
      <div v-else-if="rows.length" class="divide-y divide-slate-100">
        ...
        >
          ...
          <div class="flex items-center gap-3 min-w-0 flex-1">
            <div
              class="w-11 h-11 rounded-xl bg-orange-50/80 border border-orange-100 flex items-center justify-center shrink-0 p-1 overflow-hidden"
            >
              <img
                v-if="item.imgCode"
            ...
            <div class="min-w-0 flex-1 space-y-0.5">
              <div
                class="text-xs sm:text-sm font-bold text-slate-800 truncate group-hover:text-[#ee4d2d] transition-colors"
              >
                {{ item.productName || "Đơn hàng Shopee" }}
              </div>
              <div class="text-[11px] text-slate-400 flex items-center gap-2">
                <span class="font-mono font-medium">#{{ item.orderId }}</span>
                <span v-if="item.orderTime"
                  >·
                  {{
          ...
          <RightOutlined
            class="text-xs text-slate-300 group-hover:text-slate-500 transition-colors shrink-0"
          />
        </div>
      ...
      >
        ...
        >
          ...
          <div class="space-y-1.5">
            ...
            >
              Dán link Shopee để mua sắm và nhận hoa hồng hoàn tiền tự động nhé!
            </p>
          </div>
    ...
    >
      <div v-if="selected" class="space-y-3 text-left">
        <div
          class="p-3 bg-orange-50/70 border border-orange-200/80 rounded-xl flex items-center justify-between"
        >
          <div>
            <div class="text-[10px] font-bold uppercase text-slate-400">
              Hoa hồng của bạn
            </div>
            <div class="text-lg font-black text-[#ee4d2d]">
              {{ formatMoney(selected.userCommission) }}
            </div>
          </div>
          <span
            :class="[
              'px-2.5 py-1 rounded-full text-xs font-bold',
              getStatusBadge(selected.orderStatus).class,
            ]"
          >
        ...
        <div class="space-y-2">
          <div class="p-3 bg-slate-50 border border-slate-200/80 rounded-xl">
            ...
            <div class="text-xs font-mono font-bold text-slate-800 mt-0.5">
              {{ selected.orderId }}
            </div>
          </div>
          ...
          <div class="p-3 bg-slate-50 border border-slate-200/80 rounded-xl">
            ...
            <div class="text-xs font-bold text-slate-800 mt-0.5">
              {{ selected.productName || "Sản phẩm Shopee" }}
            </div>
          </div>
          ...
          <div class="p-3 bg-slate-50 border border-slate-200/80 rounded-xl">
            ...
            <div class="text-xs font-bold text-slate-800 mt-0.5">
              {{ formatMoney(selected.purchaseValue) }}
            </div>
          </div>

          <div
            v-if="selected.orderTime"
            class="p-3 bg-slate-50 border border-slate-200/80 rounded-xl"
          >
            <div class="text-[10px] font-bold uppercase text-slate-400">
              Thời gian ghi nhận
            </div>
            <div class="text-xs font-bold text-slate-800 mt-0.5">
              {{ new Date(selected.orderTime).toLocaleString("vi-VN") }}
            </div>
          </div>
        </div>
      </div>
