...
<template>
  <div class="w-full space-y-3 sm:space-y-4 text-left">
    ...
    <div class="bg-white rounded-2xl p-4 sm:p-5 border border-slate-200/80 shadow-xs flex items-center justify-between">
      <div class="space-y-0.5">
        ...
        <p class="text-xs text-slate-500 leading-relaxed">
          Quản lý số dư hoa hồng khả dụng, thực hiện rút tiền và theo dõi lịch sử biến động ví.
        </p>
      </div>
    ...
    <div class="bg-white rounded-2xl p-4 sm:p-5 border border-slate-200/80 shadow-xs space-y-4">
      ...
      <div class="p-3 sm:p-4 bg-slate-50/80 rounded-xl border border-slate-200/80 flex items-center justify-between gap-3">
        <div class="flex items-center gap-3 min-w-0 flex-1">
          <div class="w-9 h-9 rounded-xl bg-blue-50 text-blue-600 flex items-center justify-center shrink-0">
            <BankOutlined class="text-base" />
          </div>
          <div v-if="hasBankAccount" class="min-w-0 flex-1">
            <div class="text-[10px] font-bold text-slate-400 uppercase">Tài khoản nhận tiền</div>
            <div class="text-xs sm:text-sm font-bold text-slate-800 truncate">
              {{ savedBank?.bankName }} · {{ savedBank?.accountNo }} ({{ savedBank?.accountName }})
            </div>
          </div>
          <div v-else class="min-w-0 flex-1">
            <div class="text-xs font-bold text-amber-600">Chưa thiết lập tài khoản ngân hàng</div>
            <div class="text-[11px] text-slate-400">Vui lòng cài đặt tài khoản để nhận tiền hoa hồng</div>
          </div>
        </div>
